...
@if(Auth::check() && Auth::id() === $news->author_id)
<div class="row col-9">
<h2>
...
<form class="col-12 d-block" action="/posts/{{ $news->slug }}" method="POST">
    {{ csrf_field() }}
    @method('PATCH')
...
<form class="col-8 d-block mt-5" action="/posts/{{ $news->slug }}" method="POST">
    {{ csrf_field() }}
    @method('DELETE')
